...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const stopped = parsed.data.action === "stop";
    const state = stopped ? "stopped" : "preview_completed";
    const outcome = stopped ? "stopped" : "preview_completed";
    const stopReason = stopped ? parsed.data.stopReason : null;
    const payload = stopped
      ? { workflowVersion: current.workflowVersion, activeModuleId: parsed.data.moduleId, moduleVersions: current.modules, stopReason, principalIndex: parsed.data.principalIndex ?? null, witnessIndex: parsed.data.witnessIndex ?? null, parameters: current.parameters }
      : { workflowVersion: current.workflowVersion, activeModuleId: "FL-COMPLETE", moduleVersions: current.modules, confirmations: parsed.data.confirmations, parameters: current.parameters };
    const updated = await repository.transitionFloridaRonPreparedAttempt({ organizationId: context.organizationId, appointmentId: appointment.id, actorId: context.userId, state, outcome, stopReason, eventType: stopped ? "preview_stopped" : "preview_completed", payload });
    if (!updated) return response(404);
    return NextResponse.json({ sessionId: updated.id, state, outcome, stopReason, productionEnabled: false }, { headers: { "Cache-Control": "no-store" } });
