...
type CompletionTransition = { controlType?: "block_completion"; missingConfirmations?: string[]; state?: "preview_completed" };
...
export function AdminFloridaRonCandidatePreview({ appointmentId, available }: { appointmentId: string; available: boolean }) {
  ...
  return <section id="candidate-preview" aria-labelledby="candidate-preview-heading"><AdminCard>
    ...
    {!payload ? <div className="mt-4"><Button type="button" onClick={openPreview} disabled={loading}>{loading ? "Loading preview…" : "Enter Candidate Preview"}</Button>{error ? <p role="alert" className="mt-3 text-sm font-semibold text-red-800">{error}</p> : null}</div> : <div className="mt-5 space-y-4">
      ...
      {current?.id === "FL-COMPLETE" && !terminal ? <FinalComplianceReview content={current.content} parameters={payload.attempt.parameters} confirmed={confirmed} onChange={setConfirmed} onComplete={async () => { const result = await transition(appointmentId, { action: "preview_complete", moduleId: "FL-COMPLETE", confirmations: confirmed }); if (result?.state === "preview_completed") { setTerminal("preview_completed"); void loadHistory(); } return result; }} /> : null}
...
type StopReason = "identity" | "technology" | "willingness" | "capacity" | "incomplete_document" | "declined_consent" | "remote_witness" | "outside_florida_confirmation" | "notarial_act_not_established" | "notary_not_in_florida";
...
function FinalComplianceReview({ content, parameters, confirmed, onChange, onComplete }: { content: string; parameters: PreviewParameters; confirmed: string[]; onChange: (value: string[]) => void; onComplete: () => Promise<CompletionTransition | null> }) {
...
function completionItemApplies(item: string, parameters: PreviewParameters) { const normalized = item.toLowerCase(); if (normalized.includes("outside-florida")) return parameters.principals.some((principal) => principal.location === "outside_florida"); if (normalized.includes("witness")) return (parameters.witnesses?.length ?? 0) > 0; if (normalized.includes("117.285")) return parameters.special117285 === true; if (normalized.includes("representative")) return parameters.notarialAct === "acknowledgment_representative"; return true; }
async function transition(appointmentId: string, body: Record<string, unknown>): Promise<CompletionTransition | null> { const response = await fetch(`/api/admin/appointments/${encodeURIComponent(appointmentId)}/session-assistant/preview/transition`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const payload = await response.json() as CompletionTransition; return response.ok || payload.controlType === "block_completion" ? payload : null; }
