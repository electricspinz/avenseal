...
export function ClientPaymentCard({ token, payment }: { token: string; payment: ClientPortalViewModel["payment"] }) {
  ...
  async function beginPayment() { if (pending || !available) return; setPending(true); setMessage(""); try { const response = await fetch(`/api/appointments/access/${encodeURIComponent(token)}/payment`, { method: "POST" }); const result = await response.json().catch(() => null); if (result?.status === "checkout_ready" && typeof result.checkoutUrl === "string") { window.location.assign(result.checkoutUrl); return; } if (result?.status === "already_paid") { setAvailable(false); setMessage("Payment has already been completed."); return; } setAvailable(false); setMessage("Payment is not currently available."); } catch { setMessage("We couldn't start secure checkout. Please try again."); } finally { setPending(false); } }
