        ...
        <ul className="mt-5 space-y-3" aria-describedby="attention-heading">
          {items.slice(0, 5).map((item, index) => (
            <li key={item.id} className={`gap-3 rounded-md border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950 ${index >= 3 ? "hidden sm:flex" : "flex"}`}>
              ...
              <p><span className="font-semibold">{priorityLabel[item.priority]} · {item.category}</span><br />{item.customerName ? <span className="font-semibold">{item.customerName} · </span> : null}<span className="font-semibold">{item.title}</span> {item.description} <Link href={item.href} className="focus-ring font-semibold underline underline-offset-4">{item.actionLabel}</Link></p>
            </li>
          ))}
        </ul>
      )}
