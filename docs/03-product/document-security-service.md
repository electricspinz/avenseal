...
**Cloudmersive adapter status:** the verified basic endpoint is `POST /virus/scan/file`, authenticated only with `Apikey` and a multipart `inputFile`. The adapter sends a trusted generic filename (`document.pdf`, `document.jpg`, or `document.png`) and its declared MIME type; it never sends a customer filename, tenant or appointment identifiers, storage keys, job IDs, tokens, correlation IDs, or arbitrary metadata. It consumes only `CleanResult` and optional `FoundViruses`: `true` maps to `clean`; `false` maps to `infected`; virus details are discarded. It does not implement the advanced endpoint.

The adapter makes exactly one bounded request and has no internal retry loop. Timeout/408/425/5xx conditions are retryable; 429 is rate-limited; 401/403 are terminal authentication failures; other 4xx responses are terminal provider rejections; malformed successful payloads are terminal invalid responses. Queue-level B2C retries remain authoritative. The adapter must remain disabled in deployed environments until vendor privacy/DPA, retention/deletion, residency, size/rate, authentication, support, and incident terms are approved and staging verification is complete.
...
| --- | --- | --- |
| Read exception, missing object, empty/corrupt bytes, oversized object, signature/content mismatch, or unexpected storage client error | Retryable until five claimed attempts are exhausted | No scanner call until the worker has trusted non-empty, bounded bytes matching the declared PDF/JPEG/PNG signature; document remains quarantined and non-downloadable. |
