...
GitHub Actions invokes the processors in this order: (1) `POST /api/internal/reminders/process` promotes due reminders, then (2) `POST /api/internal/communications/process` delivers queued communications. The second request is skipped when the first fails. See [communications-scheduler.md](communications-scheduler.md) for scheduler configuration and operations.

Future SMS support can consume the same reminder records with a different queue channel.
