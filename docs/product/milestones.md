...
| PR #11 — Provider-agnostic communications engine | Active | Deliver reliable appointment communications | Queue persistence, provider boundary, email-first delivery, retries, tracking, protected batch worker, and a five-minute GitHub Actions scheduler | Appointments and secure links | SMS/push remain deferred; deployment secrets must be configured | Unit, integration, queue, scheduler, and failure-path coverage |
...
| Milestone 6 — Automated Appointment Reminders | Active | Schedule appointment lifecycle reminders | Reminder records, due promotion, cancellation/reschedule handling | Communications queue worker | External scheduler configuration pending | Unit, integration, queue idempotency coverage |
