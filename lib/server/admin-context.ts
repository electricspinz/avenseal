...
import { fallbackAvensealOrganizationId, resolvePublicOrganization } from "@/lib/server/organization";
...
export async function requireAdminOrganizationContext(): Promise<AdminOrganizationContext> {
  ...
  if (!hasSupabaseServiceConfig()) {
    ...
    if (session.email !== environment.ADMIN_DEMO_EMAIL) throw new Error("Admin organization access is required.");
    return {
