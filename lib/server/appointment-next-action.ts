...
export const appointmentNextActionKinds = ["no_action_required", "review_completion", "review_appointment", "review_payment", "review_payment_status", "waiting_for_customer_document", "security_processing", "review_document_security", "review_uploaded_document", "waiting_for_replacement_document", "prepare_session", "session_communication_processing", "review_session_communication", "session_communication_failed", "ready_for_appointment_review", "session_in_progress", "confirm_appointment_outcome", "resolve_cancelled_session"] as const;
...
export type AppointmentNextAction = Readonly<{ kind: AppointmentNextActionKind; title: string; description: string; ctaLabel?: string; targetId?: "status-management" | "payment" | "documents" | "client-workspace" | "communications" | "external-session"; href?: string; tone: AppointmentNextActionTone; context?: string }>;
...
function action(kind: AppointmentNextActionKind, title: string, description: string, tone: AppointmentNextActionTone, ctaLabel?: string, targetId?: AppointmentNextAction["targetId"], context?: string, href?: string): AppointmentNextAction { return { kind, title, description, tone, ctaLabel, targetId, context, href }; }
...
export function deriveAppointmentNextAction(input: AppointmentNextActionInput): AppointmentNextAction {
  ...
  if (input.appointmentStatus === "completed") return action("review_completion", "Review completion communications", "Review the recorded appointment outcome and completion communications.", "success", "Review communications", "communications");
  ...
  if (documents.some((document) => ["infected", "suspicious", "failed"].includes(document.scanStatus ?? "") || document.storageStatus === "removed")) return action("review_document_security", "Document security review required", "This document cannot be used for preparation. Review its recorded security state before taking any further action.", "danger", "View documents", "documents");
  ...
  if (documents.some((document) => document.status === "rejected")) return action("waiting_for_replacement_document", "Waiting for replacement document", "The customer must upload a replacement document before work can continue.", "warning", "View Client Workspace", "client-workspace");
  ...
  if (!session || session.status === "pending" || session.status === "unknown") return action("prepare_session", "Prepare notarization session", "Record the provider session details through the existing appointment controls.", "info", "Manage session", "external-session", "The approved document is available under Documents for secure provider handoff.");
  ...
  if (!communication) return action("review_session_communication", "Review session communication", "Review the existing customer session communication before treating appointment preparation as complete.", "warning", "Review communications", "communications");
  if (["queued", "processing"].includes(communication.status)) return action("session_communication_processing", "Session communication processing", "Customer session delivery is currently processing.", "info");
  if (communication.status === "failed") return action("session_communication_failed", "Session communication failed", "Use the existing Communications Center to review or retry the failed delivery.", "danger", "Open Communications Center", undefined, undefined, "/admin/communications");
  if (!["sent", "delivered"].includes(communication.status)) return action("review_session_communication", "Review session communication", "Review the existing customer session communication before treating appointment preparation as complete.", "warning", "Review communications", "communications");
