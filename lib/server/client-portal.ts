...
export function projectPortal(status: CustomerAppointmentStatus, externalSession: ExternalSession | null = null, documents: readonly CustomerDocumentStatus[] = [], readiness: CustomerReadiness = customerReadinessFromCanonical(safelyCalculateReadiness(status, externalSession, []))): ClientPortalViewModel {
  ...
  const checklist = deriveClientWorkspaceChecklist(status, session, readiness);
  return {
...
export function customerReadinessFromCanonical(readiness: AppointmentReadiness): CustomerReadiness {
  const customerStates: Record<AppointmentReadiness["state"], CustomerReadiness> = {
    ...
    waiting_for_documents: { state: "documents_needed", label: "Upload your document", explanation: "Upload the document needed for your appointment.", nextStep: "Use the Documents section below.", tone: "warning" },
    waiting_for_review: { state: "documents_under_review", label: "We’re reviewing your documents", explanation: "Your document has been received and is being securely processed and reviewed.", nextStep: "No action is needed right now.", tone: "neutral" },
    waiting_for_replacement: { state: "replacement_needed", label: "A replacement document is needed", explanation: "Please upload a replacement document so preparation can continue.", nextStep: "Use the Documents section below to upload a replacement.", tone: "warning" },
    ...
    completed: { state: "appointment_completed", label: "Appointment marked complete", explanation: "Your appointment is marked complete in Avenseal.", nextStep: "No further preparation is required.", tone: "success" },
...
export function deriveClientWorkspaceChecklist(status: CustomerAppointmentStatus, session: ClientPortalViewModel["externalSession"], readiness: CustomerReadiness) { const paid = status.paymentStatus === "paid"; const scheduled = ["confirmed", "ready", "completed"].includes(status.status); const document = readiness.state === "documents_needed" ? { state: "current" as const, detail: "Upload the document needed for your appointment." } : readiness.state === "documents_under_review" ? { state: "current" as const, detail: "Your document has been received and is being securely processed and reviewed." } : readiness.state === "replacement_needed" ? { state: "current" as const, detail: "Please upload a replacement document so preparation can continue." } : ["waiting_for_session", "ready_for_notarization", "session_in_progress", "appointment_completed"].includes(readiness.state) ? { state: "complete" as const, detail: "Your document preparation is complete." } : { state: "unavailable" as const, detail: "Document preparation needs review before this appointment can continue." }; return [{ id: "appointment", label: "Appointment scheduled", state: scheduled ? "complete" : "current", detail: deriveClientWorkspaceStatus(status) }, { id: "contact", label: "Contact information confirmed", state: "complete", detail: "Your booking contact information is on file." }, { id: "payment", label: "Payment completed", state: paid ? "complete" : status.paymentStatus ? "current" : "unavailable", detail: paid ? "Payment is recorded as received." : status.paymentStatus ? `Payment is recorded as ${paymentLabel(status.paymentStatus)}.` : "Payment status is not available." }, { id: "documents", label: "Documents prepared", ...document }, { id: "id", label: "Government-issued photo ID", state: "pending", detail: "Have your government-issued photo ID ready if required by the provider or commissioned notary." }, { id: "session", label: "Online session available", state: session.availability === "available" ? "complete" : "pending", detail: session.availability === "available" ? "Your online session is ready." : "Your online notarization session will appear here when it is ready." }] as const; }
