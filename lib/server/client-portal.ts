...
export async function queryClientPortal(token: string, dataSource: ClientPortalDependencies = dependencies): Promise<ClientPortalViewModel | null> {
  ...
  const [externalSession, documents] = await Promise.all([dataSource.getExternalSession(status.organizationId, status.appointmentId), dataSource.getCustomerDocuments?.(status.organizationId, status.appointmentId) ?? Promise.resolve([])]);
  return projectPortal(status, externalSession, documents);
}

export function projectPortal(status: CustomerAppointmentStatus, externalSession: ExternalSession | null = null, documents: readonly CustomerDocumentStatus[] = []): ClientPortalViewModel {
