-- Temporary, non-sensitive reschedule diagnostics. These stable tokens are
-- mapped server-side only and must be removed after production diagnosis.
...
begin
  if p_organization_id is null or p_appointment_id is null or p_preferred_date is null or p_preferred_time is null or p_duration_minutes is null or p_duration_minutes not between 5 and 240 then
  ...
  if v_timezone is null or v_organization_status <> 'active' or not found then raise exception 'AVENSEAL_RESCHEDULE_INACTIVE_OR_UNCONFIGURED_ORGANIZATION'; end if;
  if not exists (
  ...
  ) then raise exception 'AVENSEAL_RESCHEDULE_AVAILABILITY_SCHEDULE_MISSING'; end if;

  v_target_local := p_preferred_date + p_preferred_time;
  v_target_at := v_target_local at time zone v_timezone;
  if (v_target_at at time zone v_timezone) <> v_target_local then raise exception 'AVENSEAL_RESCHEDULE_INVALID_DST_LOCAL_TIME'; end if;
  if v_target_at < now() + make_interval(mins => coalesce(v_rules.minimum_booking_notice_minutes, 0)) then raise exception 'AVENSEAL_RESCHEDULE_MINIMUM_NOTICE'; end if;
  if not v_rules.same_day_enabled and p_preferred_date = (now() at time zone v_timezone)::date then raise exception 'AVENSEAL_RESCHEDULE_SAME_DAY_DISALLOWED'; end if;
  if v_rules.maximum_advance_booking_days is not null and p_preferred_date > (now() at time zone v_timezone)::date + v_rules.maximum_advance_booking_days then raise exception 'AVENSEAL_RESCHEDULE_BEYOND_BOOKING_HORIZON'; end if;
  ...
  ) then raise exception 'AVENSEAL_RESCHEDULE_BLOCKED_EXCEPTION'; end if;
  ...
  ) then raise exception 'AVENSEAL_RESCHEDULE_OUTSIDE_AVAILABILITY_INTERVAL'; end if;
  ...
  if v_rules.maximum_appointments_per_day is not null and v_day_count >= v_rules.maximum_appointments_per_day then raise exception 'AVENSEAL_RESCHEDULE_DAILY_LIMIT_REACHED'; end if;
  ...
  ) then raise exception 'AVENSEAL_RESCHEDULE_APPOINTMENT_OVERLAP'; end if;
  ...
  ) then raise exception 'AVENSEAL_RESCHEDULE_RESERVATION_OVERLAP'; end if;
end;
$$;
