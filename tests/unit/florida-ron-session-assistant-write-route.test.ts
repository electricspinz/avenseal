...
const mocks = vi.hoisted(() => ({ context: vi.fn(), getAppointment: vi.fn(), getSupabaseAdmin: vi.fn(), create: vi.fn(), update: vi.fn() }));
vi.mock("@/lib/server/admin-context", () => ({ requireAdminOrganizationContext: mocks.context }));
vi.mock("@/lib/server/repository", () => ({ repository: { getAppointment: mocks.getAppointment, createFloridaRonPreparedAttempt: mocks.create, updateFloridaRonPreparedAttempt: mocks.update } }));
...
function harness(current: Record<string, unknown> | null = { id: "session-1", state: "prepared", parameters: input, module_versions: originalModules, workflow_version: "FL-RON-1.0" }) {
...
describe("Florida RON session assistant write routes", () => {
  ...
  it("edits only the prepared attempt scoped to its appointment and preserves the prior snapshot in append-only audit history", async () => {
    const original = { id: "session-1", state: "prepared", parameters: input, module_versions: originalModules, workflow_version: "FL-RON-1.0" };
  ...
  it("fails closed for an unsupported act, a failed identity, and a non-prepared attempt", async () => {
    ...
    expect(await unsupportedResponse.json()).toMatchObject({ productionEnabled: false, stopReason: "notarial_act_not_established", modules: [] });
    expect(unsupported.events[0].payload).toMatchObject({ stopReason: "notarial_act_not_established" });
